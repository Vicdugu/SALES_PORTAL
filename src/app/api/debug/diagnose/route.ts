...
export async function GET(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      const stmtType = stmt.split(' ')[0] + ' ' + stmt.split(' ')[1];
      ...
      try {
        ...
        console.error(`[DIAGNOSE] ✗ Error code: ${code}`);
        console.error(`[DIAGNOSE] Error: ${errorMsg.substring(0, 100)}`);
      }
    }
